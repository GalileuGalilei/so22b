...
static err_t traduz_endereco(mmu_t *self, int end_v, int *end_f, bool escrita)
{
  ...
  return tab_pag_traduz(self->tab_pag, end_v, end_f, escrita);
}

err_t mmu_le(mmu_t *self, int endereco, int *pvalor)
{
  int end_fis;
  err_t err = traduz_endereco(self, endereco, &end_fis, false);
  if (err != ERR_OK) {
    return err;
  }
...
{
  int end_fis;
  err_t err = traduz_endereco(self, endereco, &end_fis, true);
  if (err != ERR_OK) {
    return err;
  }
