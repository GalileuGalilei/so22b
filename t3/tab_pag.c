...
tab_pag_t *tab_pag_cria(int num_pag, int tam_pag)
{
  ...
  if (self != NULL) {
    ...
    self->tab = (descr_pag_t*)malloc(sizeof(descr_pag_t) * num_pag);

    for(int i = 0; i < num_pag; i++)
    {
      self->tab[i].acessada = 0;
      self->tab[i].alterada = 0;
      self->tab[i].quadro = 0;
      self->tab[i].valida = 0;
    }
...
err_t tab_pag_traduz(tab_pag_t *self, int end_v, int *pend_f, int *ppag, int *pdesl, int *pquadro)
{
